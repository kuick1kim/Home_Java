...
ggplot(data=us_state,
  ...
  geom_polygon(color = 'black', fill = 'gray')+ coord_map() 
...
# 각 행의 이름으로 설정되어 있음.
# us_map 데이터 프레임과 조인하기 위해서는 
#주 이름들이 변수가 되어야 함. -> tibble 라이브러리
library(tibble)
...
ggplot(data = state_crime, mapping = aes(x = long, y = lat, group = group,fill = Murder))+  geom_polygon(color = 'black')+ coord_map() + scale_fill_continuous(low = 'white', high = 'darkred')
